...
export default function MedicalRecord()  
{
  ...
  async function fetchInfo()
  {
    ...
    const allergyResponse = await fetch(`http://localhost:3001/api/mr/allergy/${rep[0].MR_Id}`);
    const allergyRep = await allergyResponse.json();
    setallergyNames(allergyRep);

    const vaccinationResponse = await fetch(`http://localhost:3001/api/mr/vaccination/${rep[0].MR_Id}`);
